  ...
  useEffect(() => {
    const fetchMine = async () => {
      ...
      try {
        ...
        // keep only pending/forwarded statuses (not final)
  ...
  return (
    <div className='dashboard-container'>
        ...
        <div className='log-table'>
          ...
          {!loading && !items.length && <div style={{padding:16}}>No pending requests.</div>}
          ...
          {visibleItems.map(item => (
            <div className='table-card' key={item.id}>
              ...
              <div style={{textTransform:'capitalize'}}>{(item.status || 'pending').replace(/_/g,' ')}</div>
              <div>
                <button className='cancel-btn' style={{marginLeft:8}} onClick={async () => {
                  // optional: call cancel endpoint if exists; fallback: alert
